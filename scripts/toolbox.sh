...
BANNER
  ...
  echo ""
  g "  8) Recriar/sobe Traefik (docker stack deploy -c /opt/traefik/stack.yml traefik)"
  g "  9) Logs do Traefik (access.json tail -f)  [Ctrl+C para voltar]"
  y " 10) Reset da infra base (resetsetup.sh)"
  g " 11) Status dos serviços (docker service ls)"
  r "  0) Sair"
  echo ""
  echo -e "\033[90m──────────────────────────────────────────────\033[0m"
  echo ""
  ...
  case "$op" in
    ...
    8)  run "(cd /opt/traefik && docker stack deploy -c stack.yml traefik)" ;;
    ...
    11) echo; docker service ls --format 'table {{.Name}}\t{{.Mode}}\t{{.Replicas}}\t{{.Ports}}'; pause ;;
    0)  exit 0 ;;
